...
Three families (registered separately so individual modules stay small):
...
- ``code.snippets`` — tested code snippets + model recommendation + request
                      validation.
- ``code.scaffold`` — project scaffolders that write working starter projects
                      to disk.
"""
...
def register(mcp: FastMCP) -> None:
    ...
    from sarvam_mcp.code import docs, scaffold, snippets
